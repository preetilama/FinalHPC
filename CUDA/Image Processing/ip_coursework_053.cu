...
__global__ void detect_edges(unsigned char *in, unsigned char *out) {
  ...
  unsigned int i = (blockIdx.x*72) +threadIdx.x;
    ...
    int b;//the pixels adjacent to x,y used for calculation
    int d;//the pixels adjacent to x,y used for calculation
    int f;//the pixels adjacent to x,y used for calculation
    int h;//the pixels adjacent to x,y used for calculation
    ...
    if (x == 0 || y == 0 || x == width - 1 || y == height - 1) {
      ...
      r = (in[i] * 4) + (in[b] * -1) + (in[d] * -1) + (in[f] * -1) + (in[h] * -1);
